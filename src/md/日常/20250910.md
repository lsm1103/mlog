...
```html
<div style="padding: 10px; border: 1px solid #ccc; border-radius: 5px; text-align: center; max-width: 300px;">
    <h3>简单交互示例</h3>
    <button onclick="alert('Hello!')" style="padding: 8px 16px; background: #007bff; color: white; border: none; border-radius: 3px; cursor: pointer;">点击我</button>
    <br><br>
    <input type="range" min="0" max="100" value="50" oninput="document.getElementById('demo-value').innerText=this.value" style="width: 200px;">
    <br>
    <span>当前值: <span id="demo-value">50</span></span>
</div>
```
...
.demo-container input[type="range"]::-webkit-slider-thumb {
    -webkit-appearance: none;
    width: 20px;
    height: 20px;
    border-radius: 50%;
...
<script>
    ...
    // 处理html-render代码块
    setTimeout(function() {
        // 查找所有带有html-render标记的代码块
        const markers = document.querySelectorAll('.html-render-marker');
        markers.forEach(function(marker) {
            ...
            if (pre) {
                ...
                if (code) {
                    // 获取原始HTML内容（去除高亮标记）
                    const htmlContent = code.textContent || code.innerText;
...
```html
<div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; margin: 0; padding: 20px; background: linear-gradient(135deg,rgb(102, 126, 234) 0%, #764ba2 100%); color: white; display: flex; flex-direction: column; align-items: center; min-height: 50vh;">
    <div style="background: rgba(255, 255, 255, 0.1); backdrop-filter: blur(10px); border-radius: 20px; padding: 30px; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3); max-width: 900px; width: 100%;">
        <h1 style="text-align: center; margin-bottom: 30px; font-size: 2.2rem; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3);">抛物线垂直平移演示</h1>
        
        <div style="display: flex; flex-direction: column; align-items: center; margin-bottom: 30px; gap: 15px;">
            <div style="width: 100%; max-width: 500px; display: flex; flex-direction: column; align-items: center; gap: 10px;">
                <div style="display: flex; justify-content: space-between; width: 100%; font-size: 1.1rem;">
                    <span>向下平移量: <span id="shift-value">0</span> 单位</span>
                    <span>目标: 3 单位</span>
                </div>
                <input type="range" id="shift-slider" min="0" max="3" step="0.1" value="0" style="width: 100%; height: 8px; border-radius: 4px; background: #ddd; outline: none; -webkit-appearance: none;">
                <style>
        ...
        <div style="display: flex; justify-content: center; gap: 40px; margin-bottom: 30px; flex-wrap: wrap;">
            <div style="background: rgba(255, 255, 255, 0.2); padding: 15px 25px; border-radius: 12px; text-align: center; min-width: 200px; transition: all 0.3s ease; border: 2px solid #4ecdc4;">
                <div style="font-size: 1.5rem; font-weight: bold; margin-bottom: 5px;">y = x²</div>
                <div style="font-size: 0.9rem; opacity: 0.9;">原始抛物线</div>
            </div>
            <div style="background: rgba(255, 255, 255, 0.2); padding: 15px 25px; border-radius: 12px; text-align: center; min-width: 200px; transition: all 0.3s ease; border: 2px solid #ff6b6b;">
                <div style="font-size: 1.5rem; font-weight: bold; margin-bottom: 5px;">y = x² - <span id="current-shift">0</span></div>
                <div style="font-size: 0.9rem; opacity: 0.9;">平移后抛物线</div>
            </div>
        </div>
        
        <div style="position: relative; margin: 30px auto; width: 600px; height: 400px;">
            <canvas id="graph" width="600" height="400" style="background: white; border-radius: 12px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.2); display: block; margin: 0 auto;"></canvas>
        </div>
        
        <div style="margin-top: 20px; padding: 15px; background: rgba(255, 255, 255, 0.15); border-radius: 12px; text-align: center; font-size: 1.1rem;">
            拖动滑块观察抛物线 y = x² 向下平移的过程
        </div>
    </div>
</div>
